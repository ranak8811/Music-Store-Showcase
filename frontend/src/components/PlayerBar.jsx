    ...
    <div className="fixed bottom-0 left-0 right-0 bg-slate-900/95 backdrop-blur-md border-t border-slate-800 py-4 px-6 flex flex-col md:flex-row items-center justify-between gap-4 shadow-2xl z-50 animate-slide-up">
      ...
      <div className="flex items-center gap-3 min-w-[200px] w-full md:w-auto">
        ...
        <div className="flex flex-col min-w-0">
          <span className="text-sm font-bold text-slate-100 truncate">{activeSong.title}</span>
          <span className="text-xs text-slate-400 truncate">{activeSong.artist}</span>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center gap-4 w-full max-w-xl">
        ...
        <span className="text-xs font-mono text-slate-400">{formatTime(audioProgress)}</span>
        <input 
          type="range" 
          min="0" 
        ...
        />
        <span className="text-xs font-mono text-slate-400">{formatTime(audioDuration)}</span>
      </div>

      {/* Instrument Selection & Volume */}
